...
export default function Orders() {
  ...
  if (!user && orders.length === 0 && !loading) {
    return (
      <div className="min-h-screen bg-white pb-20 font-jakarta px-6 flex flex-col items-center justify-center text-slate-900">
        <SEO title="Track Order | Larry Printing Solutions" />
          ...
          <div className="bg-slate-50 p-8 md:p-10 rounded-2xl border border-slate-100 shadow-sm">
            <form onSubmit={handleGuestSearch} className="space-y-6">
              <div className="space-y-2 group">
                <label className="text-xs font-bold text-slate-400 ml-1">Order email</label>
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-cyan-600 transition-colors">
                    <Search size={18} />
                  </div>
                  <input 
                    type="email" required placeholder="example@email.com" value={guestEmail}
                    onChange={(e) => setGuestEmail(e.target.value)}
                    className="w-full h-12 pl-12 bg-white border border-slate-200 rounded-xl focus:border-cyan-600 outline-none text-sm font-bold transition-all placeholder:text-slate-300"
                  />
                </div>
              </div>
              <button className="w-full inline-flex items-center justify-center gap-3 bg-slate-900 text-white h-12 rounded-xl font-bold transition-all shadow-lg active:scale-95 hover:bg-cyan-600">
                Find order <ArrowRight size={16} />
              </button>
            </form>
            <div className="mt-8 pt-6 border-t border-slate-200 text-center">
              <Link to="/login" className="text-cyan-600 text-xs font-bold hover:text-slate-900 transition-colors">Sign in for full history</Link>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white font-jakarta text-slate-900 overflow-x-hidden pt-32 pb-20">
      <SEO title="Order History | Larry Printing Solutions" />
      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-10">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-40">
            ...
            <p className="text-xs font-bold text-gray-400">Loading history...</p>
          </div>
        ) : orders.length === 0 ? (
          <div className="text-center py-32 bg-slate-50 rounded-2xl border border-slate-100 max-w-2xl mx-auto">
            ...
            <Link to="/shop" className="inline-flex items-center gap-3 bg-cyan-600 text-white h-12 px-8 rounded-xl mt-8 font-bold hover:bg-slate-900 transition-all shadow-lg shadow-cyan-600/20">
          ...
          <div className="space-y-8">
            {orders.map((order) => (
              <motion.div 
                ...
                key={order.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm group hover:border-cyan-100 transition-all duration-500"
              >
                {/* Meta Header */}
                <div className="p-6 md:p-8 border-b border-slate-50 flex flex-wrap items-center justify-between gap-6 bg-slate-50/50">
                  <div className="flex flex-wrap items-center gap-8 md:gap-16">
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 mb-1">Order ref</p>
                      <h3 className="text-sm font-bold text-slate-900">#{order.order_code || order.id}</h3>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 mb-1">Placed on</p>
                ...
                <div className="p-6 md:p-8 flex flex-col lg:flex-row gap-10 items-center">
                    ...
                    {order.items && order.items.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-6 group/item">
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="text-[13px] font-bold text-slate-900 truncate">{item.product_name}</h4>
                          <p className="text-[11px] font-medium text-slate-400">Qty: {item.quantity} • ${parseFloat(item.price).toLocaleString()}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full lg:w-auto h-12 px-10 bg-slate-900 text-white rounded-xl flex items-center justify-center gap-3 text-xs font-bold hover:bg-cyan-600 transition-all active:scale-95"
                  >
              ...
              >
                <div className="flex items-center justify-between mb-10">
                  <div className="space-y-1">
                     <span className="text-[10px] font-bold text-cyan-600">Real-time update</span>
                     <h2 className="text-2xl font-bold text-slate-900 leading-none">Order <span className="text-cyan-600">journey</span></h2>
                  </div>
                  ...
                  {statusMap.map((step, idx) => {
                    ...
                    return (
                      <div key={step.key} className="relative flex gap-6 group/step">
                        <div className={cn("h-8 w-8 rounded-full flex items-center justify-center z-10 transition-all duration-500 border-2", isCompleted ? 'bg-cyan-600 text-white border-cyan-600 shadow-lg' : 'bg-white text-slate-200 border-slate-100')}>
                          <Icon size={14} />
                        </div>
                        <div className="flex-1">
                          <h4 className={cn("text-xs font-bold transition-colors duration-500", isCompleted ? 'text-slate-900' : 'text-slate-300')}>{step.label}</h4>
                ...
                <button onClick={() => setSelectedOrder(null)} className="w-full mt-10 h-12 bg-slate-50 text-slate-900 rounded-xl text-xs font-bold hover:bg-slate-100 transition-all">Close</button>
              </motion.div>
            </>
          )}
        </AnimatePresence>
